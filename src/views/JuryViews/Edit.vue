...
async function onSubmit(values, { setErrors }) {
  ...
  if (password === confirmPassword) {
    return usersStore.put(route.params.id, { username, password, firstName, lastName, email }).then(() => {
...
onMounted(() => {
  usersStore.get(route.params.id).then(() => {
    username.value = usersStore.user.username;
    password.value = usersStore.user.password;
    confirmPassword.value = usersStore.user.password;
    lastName.value = usersStore.user.lastName;
    firstName.value = usersStore.user.firstName;
    ...
    <Form @submit="onSubmit" :validation-schema="schema" v-slot="{ errors, isSubmitting }" class="space-y-4">
      ...
      <div class="relative">
        <Field name="username" type="text" id="username" class="peer text-input" :placeholder="$t('username') + '*'"
          v-model="username" :class="{ 'is-invalid': errors.username }" />
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div class="relative">
          <Field name="password" type="password" id="password" :placeholder="$t('password') + '*'" v-model="password"
        ...
        <div class="relative">
          <Field name="confirmPassword" type="password" id="confirmPassword" :placeholder="$t('confirmPassword') + '*'"
            v-model="confirmPassword" class="peer text-input" :class="{ 'is-invalid': errors.confirmPassword }" />
        ...
        <div class="relative">
          <Field name="firstName" type="text" id="firstName" class="peer text-input"
            :placeholder="$t('firstName') + '*'" :class="{ 'is-invalid': errors.firstName }" v-model="firstName" />
      ...
      <div class="flex flex-wrap justify-between mt-8">
        ...
        <button :disabled="isSubmitting" class="hover:scale-105 px-3 py-2 flex justify-center space-x-2"
          ...
          <span v-if="isSubmitting">{{ $t('editing') }}</span>
